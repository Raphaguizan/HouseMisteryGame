...
{
    public class HouseManagerFullRooms : MonoBehaviour
    {
        [SerializeField]
        ...
        private void InstantiateRoons()
        {
            ...
            // Instancia começando do topo-direito para que o primeiro quarto criado seja o topo-direito
            for (int x = matrixDim.x - 1; x >= 0; x--)
            {
                for (int y = matrixDim.y - 1; y >= 0; y--)
                {
                    var newRoom = Instantiate(roomPrefab, roonsParent).transform;
                    newRoom.localPosition = new Vector3(x * -horizontalDist, y * -verticalDist);
                    newRoom.name = $"Room({x},{y})";
                    roonsMatrix[x, y] = newRoom.GetComponent<RoomController>();
                }
            }

            // Começa DFS de geração a partir do topo-direito
            bool[,] visited = new bool[matrixDim.x, matrixDim.y];

            DFSGenerate(matrixDim.x - 1, matrixDim.y - 1, visited);
            ChooseSecretRoom();
            AddExtraDoors();
            AdjustCamerasBounds();
